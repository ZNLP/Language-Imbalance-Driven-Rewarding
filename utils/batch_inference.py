...
from inference import LlamaInferce, STOP, LANGUAGES, PROMPT_PREFIX
...
def parse_args():
    ...
    args.add_argument('--eval_type', type=str, default=None, help="evaluation type")
...
def input_format(args, d):
    if d.get('input', "") == "":
        if args.with_prefix:
            question = PROMPT_PREFIX.format(language=LANGUAGES[args.lang], question = d[args.question_key])
        else:
    ...
    with open(save_file, "w") as writer:
        for cur_data in tqdm(batch(data, args.num_per_inference), desc="Main Processing"):
            ...
            for d in cur_data:
                question = input_format(args, d)
                if d.get('input', "") == "":
                    instruction = d[args.question_key]
                else:
                    instruction = d[args.question_key] + "\n" + d['input']

                if args.eval_type == "xalpacaeval":
                    # for evaluation: x-alpacaeval
                    text = {
                        "id": d["id"],
                        "dataset": d["dataset"],
                        "prompt": question,
                        "instruction": instruction,
                        "response": jsonlines[question],
                    }
                if args.eval_type == "mgsm":
                    # for evaluation: mgsm
                    text = {
                        "prompt": question,
                        "instruction": instruction,
                        "response": jsonlines[question],
                        "final_answer": d.get("response", ""),
                    }
